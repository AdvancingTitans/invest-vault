...
股票资料整理行情、估值、财务趋势、公告和笔记；基金使用正式单位净值、阶段收益、样本回撤、规模变化、费用、基金经理、近期事件和公开持仓资料，不套用股票财报模板。资料区与“投研大师”并排，方便边核对证据边展开研究。15 位投资框架使用统一的本地插画头像；在大盘议事厅、投研大师和投研委员会中，主持席、排席名单与专家发言保持同一身份。协调员、证据研究员和报告编辑器继续使用功能头像，不冒充专家。

![投资札记 0.3.31 证券资料与投研大师](docs/images/readme-security-v0.3.31.png)
...
| 平台 | 文件 | 状态 |
|---|---|---|
| macOS Apple Silicon（M1/M2/M3/M4） | `Invest-Vault_0.3.31-local-aarch64.dmg` | 使用 ad-hoc 签名；发布包已完成构建、签名校验、DMG 校验与测试 |
...
```bash
shasum -a 256 ~/Downloads/Invest-Vault_0.3.31-local-aarch64.dmg
```
