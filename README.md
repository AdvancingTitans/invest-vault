...
市场概览集中展示 13 个 A 股、港股和美股主要指数，以及龙虎榜、行业资金流、涨停/跌停结构和最近 24 小时的大盘新闻。每个栏目可以独立刷新，刷新龙虎榜不会让其他模块跟随重载。不同市场各自保留交易日期与盘前、盘中、盘后状态；单一来源失败不会清空其他栏目。
...
| 平台 | 文件 | 状态 |
|---|---|---|
| macOS Apple Silicon（M1/M2/M3/M4） | `Invest-Vault_0.3.28-local-aarch64.dmg` | 已完成最终 DMG 签名、挂载、静态资源、公开数据刷新和本地数据保存验证 |
...
```text
5439b4a5059d144cce04532e5b0a287de368966666b542e64aa9522afca75be6  Invest-Vault_0.3.28-local-aarch64.dmg
```

在“终端”中执行：

```bash
shasum -a 256 ~/Downloads/Invest-Vault_0.3.28-local-aarch64.dmg
```
