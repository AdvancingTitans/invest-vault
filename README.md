...
**投资札记**是一款面向中国个人投资者的本地优先投资研究工作台。它不是券商终端，也不替你给出买卖指令；它把 A 股、港股和人民币基金持仓，与港股、日股、韩股、美股市场证据放进同一份可回溯档案：归档当时可见的公开事实、可复算指标、自己的判断和后续复盘。
...
- **更完整的专家证据链**：投研大师和投研委员会保留完整上游财务、基金、估值、量价、微观结构及执行证据；公开缺口按 `stock-analysis`、一手原文回填和 Agent Reach 顺序补强。
...
- 同时持有 A 股、港股或基金，又需要观察日股、韩股、美股和跨市场风险的中国投资者。
...
大盘议事厅是与今日复盘平级的一级工作台，不嵌在 A股概览或全球概览内。它读取当前归档的全部相关市场证据，而不是只读取页面上可见的几行；开始议事不会强制刷新或改变市场页面。单专家或六席投研委员会会先核对当前交易时段，再把本地持仓作为条件化观察语境，不输出交易指令。
...
笔记支持标题、强调、引用、列表和表格等 Markdown 语义，但阅读界面不会暴露原始标记。持仓笔记与大盘概览笔记分开管理；市场笔记再按盘前、盘中、盘后三个明确阶段归档，不根据创建时间猜测阶段。投研委员会或单专家的最终报告与中间结论保留确定性标题。
...
| 平台 | 文件 | 状态 |
|---|---|---|
| macOS Apple Silicon | `Invest-Vault_0.3.34-local-aarch64.dmg` | 使用 ad-hoc 签名；发布包已完成构建、挂载、签名、冻结服务与 DMG 校验 |
...
```text
886dccb977a36dab6e4df0e1be824d0a3868f0e92980bdc4db5bd14884552977  Invest-Vault_0.3.34-local-aarch64.dmg
```

在“终端”中执行：

```bash
shasum -a 256 ~/Downloads/Invest-Vault_0.3.34-local-aarch64.dmg
```
...
- AI 每次装配当前任务所需的完整证据包，不上传整个数据库或附件目录；证据、专家意见与最终综合之间不再按字符数截断。
...
- 大盘议事厅按所处时段，结合当前所有可用且完整的证据生成报告，并结合本地持仓给出条件化观察建议。A股、港股、日股、韩股、美股及应用内未展示的相关公开、计算和持仓证据均可进入；页面模块不是证据上限。专家先按内置 `stock-analysis` 路由补齐框架证据，公开缺口再由 Agent Reach 只读检索；只有输入充分、口径准确且业界通行的二次计算才会作为明确标注的计算证据。开始议事不强制刷新市场页面，页面刷新也不禁用或重置议事厅、投研大师和投研委员会。
